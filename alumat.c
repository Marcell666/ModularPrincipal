...
*	  1.0	  Flávio   06/11/2017	Produto final para entrega	
...
*     Estrutura principal que armazena os dados de um aluno matriculado ao qual este módulo se dedica.
*
***********************************************************************/
...
AMT_tpCondRet AMT_alteraTurma(unsigned int mat, char* codDisciplina, char* codTurma) {
	...
	Turma *turma;
	InfoGradeHorario *i = NULL;
	if(AMT_BuscaAluno(mat, &AluGrad) != AMT_CondRetAlunoJaCadastrado) // Busca para saber se aluno está matrículado em alguma disciplina.
		return AMT_CondRetAlunoNaoExiste;
...
/*Fim da função*/
/*Função que recebe uma matrícula e remove aquele aluno da lista dos alunos matriculados*/
AMT_tpCondRet AMT_deletaNo(unsigned int mat){
	...
	for (i = 0;i < size; i++) {
		get_val_cursor(AluMatr->AMT, (void**)&parAluGrade);	// Pego o par aluno-grade atual
		ALU_GetMat(parAluGrade->aluno, &mat2);	// Pego a matrícula do aluno
		if (mat == mat2) {	// Vejo se a matrícula é igual à de busca
...
/*Início da função limpa*/
/*Função que remove todos os nós da lista dos alunos matrículados e desaloca sua memória*/
